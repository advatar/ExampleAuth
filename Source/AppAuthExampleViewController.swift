...
*/
let kIssuer: String = "https://api.svipe.com/oidc/v1/";
...
*/
let kClientID: String? = "deaf2cb5-136c-44e4-a2de-ee0c97426ec5";
...
/**
 ...
 For client configuration instructions, see the [README](https://github.com/openid/AppAuth-iOS/blob/master/Examples/Example-iOS_Swift-Carthage/README.md).
*/
let kRedirectURI: String = "appauth://oauth2redirect/svipe";
...
extension AppAuthExampleViewController {
    ...
    @IBAction func authWithAutoCodeExchange(_ sender: UIButton) {
        ...
        self.logMessage("Fetching configuration for issuer:\n \(issuer)")
    ...
    @IBAction func authNoCodeExchange(_ sender: UIButton) {
        ...
        guard let issuer = URL(string: kIssuer) else {
            self.logMessage("Error creating URL for :\n \(kIssuer)")
            return
        }
        ...
        OIDAuthorizationService.discoverConfiguration(forIssuer: issuer) { configuration, error in
            ...
            guard let configuration = configuration else {
                self.logMessage("Error retrieving discovery document.\n Error & Configuration both are NIL!")
                return
            }
    ...
    @IBAction func codeExchange(_ sender: UIButton) {
        ...
        OIDAuthorizationService.perform(tokenExchangeRequest) { response, error in
            ...
            if let tokenResponse = response {
                self.logMessage("Received token response with accessToken:\n \(tokenResponse.accessToken ?? "DEFAULT_TOKEN")")
            } else {
    ...
    @IBAction func userinfo() {
        ...
        self.authState?.performAction() { (accessToken, idToken, error) in
            ...
            if error != nil  {
                self.logMessage("Error fetching fresh tokens:\n \(error?.localizedDescription ?? "ERROR")")
                return
            }
...
extension AppAuthExampleViewController {
    ...
    func saveState() {
        ...
        if let userDefaults = UserDefaults(suiteName: "group.com.svipe.appauth.Example") {
    ...
    func loadState() {
        guard let data = UserDefaults(suiteName: "group.com.svipe.appauth.Example")?.object(forKey: kAppAuthExampleAuthStateKey) as? Data else {
            return
        }
    ...
    func updateUI() {
        ...
        if let authState = self.authState {
            self.authAutoButton.setTitle("Login with Svipe", for: .normal)
        } else {
            self.authAutoButton.setTitle("Login with Svipe", for: .normal)
        }
    }
